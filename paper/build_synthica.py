...
class PaperPDF(FPDF):
    def header(self):
        if self.page_no() > 1:
            ...
            self.cell(0, 4, "Leakage-controlled cross-platform study", align="L")
...
def build():
    ...
    pdf.body_text("GEO GSE39582: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE39582. TCGA-COAD: https://xenabrowser.net/. CPTAC-COAD: https://cptac-data-portal.georgetown.edu/. Code and trained pipelines are available from the corresponding author upon request.")
    ...
    output = str(base / "paper" / "synthica_manuscript.pdf")
    pdf.output(output)
