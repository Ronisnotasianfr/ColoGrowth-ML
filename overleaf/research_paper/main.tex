...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\small Cross-Platform Colon Cancer Proliferation Classification}
\fancyfoot[C]{\thepage}
\doublespacing
...
Tumor proliferation is a major driver of aggressiveness and survival in colorectal cancer, and gene-expression data could offer a quantitative, platform-independent surrogate for Ki-67 staining. However, machine-learning pipelines for transcriptomics often leak label information into feature selection and routinely fail across platforms. I built a leakage-free framework that classifies high- versus low-proliferation status of colon adenocarcinoma and tested whether it generalizes to independent platforms.

I trained four classifiers (logistic regression, random forest, XGBoost, neural network) on GEO GSE39582 ($n = 585$) after removing the ten cell-cycle genes that define the target, wrapping all preprocessing in scikit-learn Pipelines with nested cross-validation so training and validation never mixed. External validation used TCGA-COAD RNA-seq ($n = 322$) and CPTAC-COAD proteome ($n = 105$) after quantile normalization and Platt calibration. Survival used Kaplan-Meier and Cox regression (with a formal power analysis), drugs a GDSC2 screen (295 agents, Bonferroni-corrected Mann-Whitney $U$), feature recovery a synthetic 20-gene experiment, and biological fidelity a correlation of model scores with Ki-67 IHC MKI67 expression. A LASSO minimal model tested how small a leakage-free panel can be.

Hold-out ROC-AUC reached 0.981--0.991 (XGBoost 0.991); random forest reached 0.973 AUC / 0.921 accuracy on TCGA and 0.949 / 0.868 on CPTAC. Model proliferation scores correlated strongly with Ki-67 expression (GEO $r = 0.589$, $p = 5.4 \times 10^{-56}$; TCGA $r = 0.543$, $p = 1.2 \times 10^{-26}$). High proliferation predicted shorter survival (log-rank GEO $p = 0.037$, TCGA pan-cancer $p = 0.009$) but not after Cox adjustment for stage (HR $= 0.84$, $p = 0.31$); the analysis was adequately powered for these cohorts. Trametinib was the top drug ($p = 1.8\times 10^{-12}$, all top hits MEK inhibitors). Synthetic ground truth recovered 20/20 signal genes at 7.7$\times$ enrichment. Ablation showed StabilitySelector $\geq$ SelectKBest for all models (MLP +0.009 AUC), and a LASSO panel of 83 genes matched the full 500-gene pipeline (AUC 0.985 vs.\ 0.983), with KIF23, DNA2, and MCM3 the most stable. Leakage-free classifiers generalize across platforms, but high AUC partly reflects an easy task (Cohen's $d = 2.3$), and proliferation adds no independent survival value beyond staging.

\textbf{Keywords:} colon cancer; proliferation; machine learning; data leakage; cross-platform validation; calibration; Ki-67; drug sensitivity; survival analysis

\newpage

\section{Introduction}

Proliferation rate --- how quickly a tumor's cells divide --- is one of the strongest indicators of aggressiveness and outcome in colorectal cancer. In the clinic it is usually estimated from Ki-67 immunostaining or from tumor staging, and both tools have known limitations \citep{zeng2025}. Ki-67 scoring varies between pathologists, while staging is a coarse anatomical measure that does not capture the molecular machinery driving cell division. Meanwhile, transcriptomic profiling has become routine: microarrays and RNA sequencing both read out the expression of the cell-cycle genes that control division, and tumor-specific signatures such as the consensus molecular subtypes \citep{guinney2015} and the TCGA genomic landscape of colon adenocarcinoma \citep{tcga2012} provide rich biological context. What is missing is a quantitative proliferation score computed directly from expression data that survives a change of platform.

Machine learning is a natural fit, but transcriptomic-prediction studies carry two recurring weaknesses. The first is data leakage: when the label is defined by a set of genes, those same genes often remain in the feature matrix, and the model learns to recite the label instead of learning the biology. The second is a lack of cross-platform validation: a classifier tuned to a microarray's dynamic range may collapse completely when applied to RNA-seq or protein-level data \citep{vanclaster2019}. I designed this study to address both weaknesses directly.
...
\textbf{Hypothesis.} A leakage-free classifier trained on GEO (GSE39582) achieves ROC-AUC $>$ 0.90 on the external RNA-seq (TCGA-COAD) and proteogenomics (CPTAC-COAD) cohorts after calibration, model scores correlate with Ki-67 expression, and the proliferation prediction carries independent prognostic value beyond clinical staging.

\section{Materials and Methods}

\subsection{Datasets and Preprocessing}
The GEO processed dataset GSE39582 \citep{marisa2013} contains 585 samples and 22,182 features after removal of the ten signature genes, with age, sex, and stage as clinical covariates. The binary proliferation target is balanced (292 low / 293 high). An 80/20 stratified split produced a training pool of 468 samples and a hold-out test set of 117 (Table \ref{tab:data}). External validation used TCGA-COAD RNA-seq \citep{tcga2012} ($n = 322$ after the calibration split) and the CPTAC-COAD mass-spectrometry proteome \citep{vasikar2019} ($n = 105$). The independent GEO cohort GSE17538 \citep{smith2010} was reserved for survival power analysis.
...
The proliferation target was defined as the mean $z$-score of ten core cell-cycle genes (MKI67, PCNA, TOP2A, MCM2, MCM6, AURKA, BUB1, CCNB1, CDK1, BIRC5), following the cell-cycle catalogue of \citet{whitfield2002}, binarized at the cohort median. Critically, these ten genes were removed from the feature matrix before any split, so the classifiers can never reconstruct the label from the genes that define it. All preprocessing (variance threshold 0.01, standard scaling, feature selection) was wrapped inside scikit-learn Pipelines \citep{pedregosa2011}, and evaluation used nested cross-validation: the inner loop selected features within each training fold, the outer loop evaluated on the held-out fold only. Hyperparameters were optimized by GridSearchCV inside the inner folds (Table \ref{tab:hyper}).
...
XGBoost \citep{chen2016} & n\_estimators & 200 & 200 \\
...
Instead of selecting features once on the full training set (a subtle leakage source), I used a bootstrap-based stability selector in the spirit of stability selection \citep{meinshausen2010}: features are ranked by ANOVA $F$-score on each of $B$ bootstrap resamples of the training fold, and a feature is retained only if it lands in the top $K$ across at least 50\% of resamples. This yields a feature set that is robust to sampling noise and consistent across folds; the production pipeline used $B = 30$ and $K = 500$. An ablation study ($B = 50$) compared the selector against plain SelectKBest (Section \ref{sec:ablation}).

\subsection{Models and Training}
Four classifiers were compared: L2-regularized logistic regression, random forest, XGBoost \citep{chen2016}, and a two-hidden-layer MLP, all trained inside identical pipelines so that any performance difference reflects the model, not the preprocessing. Outer-loop 5-fold cross-validation reported the training performance; the untouched 20\% hold-out ($n = 117$) provided the internal test.

\subsection{Cross-Platform Calibration}
Microarray and RNA-seq instruments have different dynamic ranges, so raw model probabilities are not directly comparable across platforms \citep{vanclaster2019}. Four calibration strategies were benchmarked: Platt scaling \citep{platt1999}, isotonic regression \citep{zadrozny2002,niculescu2005}, quantile normalization (QN) alone, and QN combined with Platt. For every external cohort, half of the cohort was used to fit the calibration mapping and the other half to evaluate; the assignment was stratified and the split later merged for the final metrics. Expected calibration error (ECE) was bootstrap-estimated.

\subsection{Statistics, Survival, and Drug Screen}
Hold-out performance is reported with 95\% confidence intervals from 1,000 bootstrap resamples \citep{efron1993}; pairwise model differences use bootstrap $p$-values. Survival was compared with the Kaplan-Meier estimator and log-rank test \citep{kaplan1958} between predicted classes, and with a multivariable Cox proportional-hazards model adjusted for age, sex, and stage \citep{cox1972}; statistical power for each cohort was assessed following \citet{schoenfeld1983}. A GDSC2 drug screen \citep{yang2013,iorio2016} compared drug sensitivity of high- versus low-proliferation colon lines with Mann-Whitney $U$ tests \citep{mann1947} (Bonferroni-corrected over 295 drugs; $\alpha = 0.05/295$). Feature-level interpretation used SHAP values \citep{lundberg2017}, pathway enrichment \citep{subramanian2005} against Gene Ontology \citep{ashburner2000} and KEGG \citep{kanehisa2000}, and clinical decision curve analysis \citep{vickers2006}. Finally, a synthetic experiment (300 samples, 2,000 genes, 20 planted signal genes) tested whether the pipeline recovers true signal rather than noise, and a LASSO model \citep{tibshirani1996} tested how small a leakage-free panel can be while matching full-pipeline performance.

\section{Results}

\subsection{Hold-out Performance}
Table \ref{tab:cv} summarizes the nested-CV and hold-out results. Mean CV ROC-AUCs were 0.9801--0.9832 for logistic regression and random forest, 0.9756 for XGBoost, and 0.9711 for the MLP. On the hold-out ($n = 117$), XGBoost had the best hold-out ROC-AUC (0.9910); random forest achieved the best accuracy (0.9273). Calibration curves (Figure \ref{fig:calibration}) show the models are reasonably calibrated, with tree-based models closest to the diagonal.
...
After QN + Platt calibration, random forest reached an AUC of 0.973 and accuracy 0.921 on TCGA-COAD RNA-seq (XGBoost: 0.968 / 0.903). On CPTAC-COAD (proteome, $n=105$), all four models stayed above 0.88 after calibration, with random forest best (0.949 / 0.868) and MLP close behind (0.940 / 0.830) (Table \ref{tab:cpac}). A classifier trained purely on Affymetrix microarrays therefore transfers to RNA-seq and mass-spectrometry platforms with only a modest drop.
...
Across the four external protocols, Platt scaling reduced ECE most for tree ensembles (random forest: ECE 0.115 $\rightarrow$ 0.043, 95\% CIs non-overlapping), consistent with the observation that tree ensembles rank well but miscalibrate absolute probabilities \citep{niculescu2005}. For logistic regression, QN + Platt gave the best cross-platform transfer (AUC 0.972). Table \ref{tab:calib} reports the best configuration per model.
...
\midrule
MCM10 & 886.28 & DNA replication prereplication \citep{langston2017} \\
...
As a leakage-free biological check, model predictions were correlated with expression of MKI67 itself --- the gene encoding the Ki-67 protein used in clinical IHC, which was deliberately held out of training. Model proliferation scores correlated strongly with MKI67 in both cohorts (GEO GSE39582: $r = 0.589$, $p = 5.4 \times 10^{-56}$; TCGA-COAD: $r = 0.543$, $p = 1.2 \times 10^{-26}$; Figures \ref{fig:ki67geo} and \ref{fig:ki67tcga}). Because the model cannot see MKI67, this correlation must emerge from the downstream transcriptional cascade, supporting the claim that the classifier captures genuine proliferation biology rather than a memorized label signature --- and offering an objective, reproducible surrogate for subjective Ki-67 IHC scoring \citep{zeng2025}.
...
Checks whether the model ``fails'' for particular age, sex, or stage subgroups did not show any subgroup-specific collapse; interaction tests were all insignificant (Table \ref{tab:subgroup}).
...
To interpret these null results, a formal power analysis was performed across nine cohorts following \citet{schoenfeld1983} (Table \ref{tab:power}). The GEO, TCGA, and pan-cancer cohorts have adequate event counts ($\geq 79$ events) to detect a log-rank difference at $\alpha = 0.05$, so the significant univariate findings are not artifacts of underpowering; CPTAC-COAD (7 events) and TCGA-READ (21 events) are demonstrably underpowered, and their null results should not be read as evidence against the association.
...
To check whether the pipeline finds real regulatory signal, I created a synthetic matrix (300 samples, 2,000 genes) in which exactly 20 genes drove the label. All four models recovered all 20 genes (recall $=$ 1.0, 7.7$\times$ enrichment), with best AUC 0.988 (random forest) (Table \ref{tab:synthetic}).
...
As a translation-minded analysis (in the spirit of clinical minimal panels such as ColoGuideEx \citep{agesen2012} and recurrence signatures \citep{oconnell2010}), I fit a LASSO logistic-regression model \citep{tibshirani1996} inside the same leakage-free pipeline ($C = 0.1$ by CV). The LASSO selected $n = 83$ genes and reached hold-out AUC 0.9849 / accuracy 0.9152 --- statistically indistinguishable from the full 500-feature pipeline (0.9834; Table \ref{tab:lasso}). Stability was assessed by 30 bootstrap resamples: 30 genes were selected in $\geq 50\%$ of resamples, and the most stable genes (Table \ref{tab:lassostab}) are all replication or mitosis machinery (KIF23 cytokinesis, DNA2 helicase/repair, MCM3 replication licensing), independently confirming the pathway-level results of Section \ref{sec:biology}.
...
A leakage-free pipeline separates colonic high- and low-proliferation reliably (hold-out AUC 0.9812--0.9910), and after QN + Platt the models generalize to genuinely external data on different platforms (TCGA RNA-seq, CPTAC proteome). Calibration is easiest for tree ensembles (low ECE after Platt), while QN + Platt was the best protocol for logistic regression. Feature selection was highly stable (identical top-20 across folds), the synthetic experiment shows the pipeline recovers true rules, performance is homogeneous across subgroups, and an 83-gene LASSO panel matches the full pipeline --- all independent lines of evidence that the learned signal is biological, not an artifact of leakage or platform.

\subsection{Independent Biological Evidence: Ki-67 and Pathways}
The two strongest pieces of biological evidence are the correlation of model scores with MKI67 (the clinically used Ki-67 gene, held out of training: $r = 0.59$ GEO, $r = 0.54$ TCGA) and the enrichment of DNA-replication, cell-cycle, and mitotic-spindle pathways --- including replication-licensing and kinetochore components independently flagged by the LASSO panel (KIF23, DNA2, MCM3). Together they suggest the classifier is a viable objective surrogate for subjective Ki-67 IHC \citep{zeng2025} and could be paired with therapeutic readouts: the GDSC2 screen links the high-proliferation class to MEK-pathway inhibition, hinting that an expression-derived proliferation score has actionable therapeutic meaning in colon cancer cell lines \citep{iorio2016}.

\subsection{Why the AUC Is So High --- Effect Size}
The high AUCs must be read with a speed limit. The proliferation median split produces two classes separated by a Cohen's $d$ of 2.3 (barely overlapping) \citep{cohen1988}, and proliferation is a many-gene, broadly correlated process: thousands of genes beyond the ten removed correlate with the label. The task is therefore easier than typical clinical ML problems, so AUC around 0.99 does not mean the model is clinically magic --- it means the classes are separable by structure. Reporting effect size alongside AUC is an honest-practice point promoted here explicitly.
...
I demonstrated that a leakage-free machine-learning pipeline can classify colon cancer proliferation status from transcriptomics and generalize to independent RNA-seq and proteomics platforms with AUC above 0.94. Model scores correlate with Ki-67 expression despite the Ki-67 gene being held out, an 83-gene LASSO panel matches the full pipeline, and proliferation carries a large effect size (Cohen's $d = 2.3$), an essential context for interpreting the high AUC; it does not provide independent survival information beyond stage. Controlling leakage, calibrating cross-platform pipelines, and reporting effect size alongside AUC are the main methodological contributions and are all reproducible from public data and open code.
...
\section*{Acknowledgments, Ethics, and AI Disclosure}
Secondary analysis of public de-identified data does not require ethics approval; the model is for research use only. Claude (Anthropic) assisted with code debugging, table formatting, and the manuscript assembly; the study design, analyses, and scientific interpretations are the author's own. Prompt logs, code, and data are in the repository.
